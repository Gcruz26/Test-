...
export function ReportSummariesPage({ initialIntakeId = null, requestedIntakeId = null }: Props) {
  ...
  return (
    <section className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-[linear-gradient(135deg,_#101827,_#1a3150_52%,_#0b6a77)] px-5 py-3.5 text-slate-50 shadow-sm">
        <h1 className="text-lg font-semibold tracking-tight">Report Summaries</h1>
        <span className="rounded-full border border-white/10 bg-white/8 px-3 py-1 text-xs font-medium text-cyan-100">
          {currentIntake ? `#${currentIntake.id} • ${currentIntake.client_name} • ${currentIntake.client_platform}` : "No intake selected"}
        </span>
      </div>
              ...
              <div className="grid gap-3">
                <div className="grid gap-2">
                  ...
                  <select
                    ...
                    name="client-filter"
                    className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
                  >
        ...
        <div className="grid gap-3">
          <Card className="border-slate-200/90 bg-white/90">
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-sm text-slate-950">Processed summary</CardTitle>
                  <CardDescription>
                    {currentIntake
                ...
                <>
                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      <strong className="block text-slate-950" style={{ fontVariantNumeric: "tabular-nums" }}>{formatNumber(currentInterpreterSummaries.length)}</strong>
                      Summary rows
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      <strong className="block text-slate-950" style={{ fontVariantNumeric: "tabular-nums" }}>{formatNumber(currentInterpreterSummaries.filter((row) => row.matched).length)}</strong>
                      Matched rows
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      <strong className="block text-slate-950" style={{ fontVariantNumeric: "tabular-nums" }}>{formatNumber(currentUnmatched.length)}</strong>
                      Unmatched rows
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    ...
                    <div className="grid gap-2">
                      ...
                      <select
                        ...
                        name="match-filter"
                        className="h-11 min-w-[180px] rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
                      >
                        ...
                        <tbody>
                          {currentInterpreterSummaries.map((row, index) => (
                            <tr key={`${row.client}-${row.client_id}-${row.interpreter_name}-${index}`} className={`border-b border-slate-200 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/60"}`}>
                              ...
                              <td className="px-4 py-3 text-right text-slate-700" style={{ fontVariantNumeric: "tabular-nums" }}>{formatNumber(row.total_calls)}</td>
                              <td className="px-4 py-3 text-right text-slate-700" style={{ fontVariantNumeric: "tabular-nums" }}>{formatMinutes(row.total_minutes)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  {currentUnmatched.length > 0 ? (
                    <div className="grid gap-3">
                      <div className="flex items-center gap-2">
                        <Table2 className="size-4 text-slate-500" aria-hidden="true" />
